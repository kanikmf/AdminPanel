...
namespace InfinityPlatform.MvcUl.Areas.AdminPanel.Controllers
{
    ...
    public class ClientController : Controller
    {
        ...
        [HttpPost]
        public async Task<IActionResult> Update(int id, UpdateClientDto updateClientDto, IFormFile clientImage)
        {


            if (clientImage == null)
                return Json(new { IsSuccess = false, Message = "Müşteri resmi seçilmelidir" });
            ...
            var response =   await _apiService.UpdateData("/clients/", JsonSerializer.Serialize(postData));
            if (response)
                return Json(new { IsSuccess = true, Message = "Müşteri Güncellendi" });

            return Json(new { IsSuccess = false, Message = "Müşteri Güncellenmedi" });
        }

...
namespace InfinityPlatform.MvcUI.ApiServices
{
    public class HttpApiService : IHttpApiService
    {
        ...
        public async Task<bool> UpdateData(string requestUri, string jsonData, string token = null)
        {          
            var requestMessage = new HttpRequestMessage()
            {
                ...
                Headers =
        {
          {HeaderNames.Accept,"application/json" }
            ...
            var responseMessage = await client.PutAsync(requestMessage.RequestUri,requestMessage.Content);
